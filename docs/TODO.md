...
#### 実測: 内部 I2C のデバイス一覧とパネル世代（2026-09-22）

内部 I2C (G31/G32) を 0x08..=0x77 でスキャンした結果:

```
i2c0 scan: 0x10 0x14 0x32 0x40 0x41 0x43 0x44 0x68
```

| アドレス | 意味 |
|---|---|
| **0x14** | **GT911 タッチ** → この個体は **ILI9881C 世代**（下記） |
| 0x43 | PI4IOE5V6408。LCD_EN と電源。ポートが使っている方 |
| **0x44** | PI4IOE5V6408-2。**SHT31 の既定アドレスと衝突することを実機で確認** |
| 0x68 | BMI270 IMU |
| 0x10 / 0x32 / 0x40 / 0x41 | コーデック (ES8388 / ES7210)・RTC (RX8130CE)・INA226 に相当。
  個別の同定はしていない |

**Tab5 のパネル駆動 IC は製造時期で変わる**（公式ページ）:
ILI9881C → **ST7123**（2025-10-14 以降）→ **ST7121**（2026-04-28 以降）。
タッチのアドレスで世代が分かり、GT911 (0x14) なら ILI9881C 世代、
0x55 なら ST7123 / ST7121 世代。**この個体は 0x14 なので ILI9881C**。
内蔵ディスプレイを実装するときは、この個体向けの初期化シーケンスが
他の個体でそのまま動くとは限らないことに注意する。
...
- [x] **`ports/esp32p4` の I2C / SPI**。PORT.A を `i2c.bus` index 0、
      M5-Bus の SPI2 (SCK=G5 / MOSI=G18 / MISO=G19) を `spi.bus` index 0 として
      実装した（abi-spec §8 の表どおり）。周波数とモードは `spi.bus.open` で
      `apply_config` に渡す。`spi.transfer` は esp-hal がその場入れ替えなので
      `data` を `buf` へ写してから渡し、長さは短い方に合わせる（host と同じ規則）。
      **バスのピンは `reserved` に入れていない**（PORT.A の I2C と同じ扱い）。
      ゲストが同じ番号を素の GPIO として開くと競合するのは I2C でも同じ
...
- [x] **P4 の f32 は host (AArch64) と一致した**（2026-09-21）。sensor-display の
      温度バーを含む 1078 行が一致しているので、P4 の単精度ハード FPU (RV32IMAFC) と
      host で差は出ていない。**RP2040 のソフトフロートと ESP32-S3 の Xtensa FPU は
      未測定**なので、3 実装を跨いだ一致はまだ言えない
